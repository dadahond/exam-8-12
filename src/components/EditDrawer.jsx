...
function EditDrawer() {
  ...
  const updateInvoice = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Error occured while refreshing!");
      }
      handleReload();
    } catch (error) {
      console.error(error);
    ...
    <div>
      <div className="drawer ">
        ...
        <form
          ...
          ref={formRef}
          className="drawer-side ml-[95px] h-full"
        >
          <label
            htmlFor="my-drawer"
          ...
          <div>
            <ul className=" min-h-full ">
              <div className="max-w-3xl mx-auto p-14  rounded-lg bg-lightMist dark:bg-midnightBlack ">
                ...
                <button
                  onClick={addNewItem}
                  type="button"
                  className="btn w-full py-2 mt-4  mb-4 rounded-lg text-darkSlate"
                >
                  + Add New Item
                </button>
              </div>
              <div className="flex justify-end sticky bottom-0 left-0 p-8 bg-darkSlate">
